...
- A session spawned in shared mode has no persistent worktree, so it cannot be respawned. The UI hides the Respawn button via `canRespawn(t.workspace)` (true only for `Worktree`), so respawn is not normally invoked for shared sessions. If it ever were, `run_respawn` plans worktree mode and its `test -d` preflight finds no surviving worktree, returning `SessionNotFound`; `plan_spawn`'s `NotWorktreeProject` remains only as a defensive guard on the now-unreachable shared-plan path. Shared-mode sessions remain effectively single-writer.
